...
def estandarizar_palabra(palabra: str):
...
def concatenar_fila(fila: pd.Series):
    fila = fila.astype(str).str.lower().str.normalize('NFKD').str.encode(
        'ascii', errors='ignore').str.decode('utf-8')
    return ' '.join(fila)


def es_fecha(string, fuzzy=True):
    """
...
def parse_number(text):
    ...
    if match is None or not (match.group("integer_part")
                             or match.group("decimal_part")):  # failed to match
...
def parse_rut(rut):
    ...
    return "{}-{}".format(
        rut_formateado,
        digito_verificador if digito_verificador != 10 else 'K').replace('.', '')
...
def compare_results(validation_data: pd.DataFrame, results: Dict, verbose: bool = True):
    ...
    for results_col, results_value in results.items():
        ...
        try:
            ...
            print(f'{bcolors.WARNING}[NO EXISTE]: No existe la columna {results_col}'
