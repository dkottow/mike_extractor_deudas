...
from ..commons.utils import estandarizar_palabra, concatenar_fila, es_fecha, parse_number, parse_rut


class DDATablesProcessor():
    def __init__(self, tablas_pagos: List[pd.DataFrame]):

        if len(tablas_pagos) > 1:
    ...
    def encontrar_encabezado(self, tabla_pagos):
        ...
        for idx, row in tabla_pagos.iterrows():
            ...
            if all([header in fila_como_str for header in headers_a_buscar]):
                ...
                columns = [estandarizar_palabra(col) for col in columns]

                # Fijar nombres de las columnas
                tabla_pagos.columns = [
                    'cuota', 'fvcto', 'factura', 'estado', 'fecha_pago', 'valor_cuota',
                    'monto_actual', 'amortiz', 'intereses', 'saldo_capital'
                ]
    ...
    def get_fila_ultima_cuota_pagada(self) -> Union[pd.Series, None]:
        try:
            for idx, row in self.tabla_pagos.iterrows():
                if not es_fecha(row['fecha_pago']):
                    if (idx == 0):
                        return None
            ...
            logging.error('{} error: {}'.format(
                DDATablesProcessor.get_fila_ultima_cuota_pagada.__qualname__, e))
            return None
    ...
    def get_fila_primera_cuota_impaga(self) -> Union[pd.Series, None]:
        ...
        try:
            ...
            logging.error('{} error: {}'.format(
                DDATablesProcessor.get_fila_primera_cuota_impaga.__qualname__, e))
            return None
    ...
    def get_ultima_cuota_pagada(self) -> Union[int, None]:
        try:
            ...
            logging.error('{} error: {}'.format(
                DDATablesProcessor.get_ultima_cuota_pagada.__qualname__, e))
    ...
    def get_primera_cuota_impaga(self) -> Union[int, None]:
        try:
            ...
            logging.error('{} error: {}'.format(
                DDATablesProcessor.get_primera_cuota_impaga.__qualname__, e))
    ...
    def get_valor_cuota(self) -> Union[int, None]:
        try:
            ...
            logging.error('{} error: {}'.format(
                DDATablesProcessor.get_valor_cuota.__qualname__, e))
    ...
    def get_total_pagare(self) -> Union[int, None]:
        try:
            ...
            logging.error('{} error: {}'.format(
                DDATablesProcessor.get_total_pagare.__qualname__, e))
    ...
    def get_monto_impago(self) -> Union[int, None]:
        try:
            if self.fila_ultima_cuota_pagada is not None:
                return parse_number(
                    self.fila_ultima_cuota_pagada['saldo_capital'].replace('.', ''))
            ...
            logging.error('{} error: {}'.format(
                DDATablesProcessor.get_monto_impago.__qualname__, e))
            self.errors.append('Monto Impago')
            return None

    def get_fecha_pago_primera_cuota(self) -> Union[datetime.datetime, None]:
        try:
            return parse_date(self.tabla_pagos.loc[0, 'fvcto'],
                              fuzzy=True,
                              dayfirst=True)
        except Exception as e:
            logging.error('{} error: {}'.format(
                DDATablesProcessor.get_fecha_pago_primera_cuota.__qualname__, e))
    ...
    def get_dia_pago(self) -> Union[int, None]:
        try:
            ...
            logging.error('{} error: {}'.format(
                DDATablesProcessor.get_dia_pago.__qualname__, e))
            self.errors.append('Día Pago')
            return None

    def get_fecha_mora(self) -> Union[datetime.datetime, None]:
        try:
            return parse_date(self.fila_primera_cuota_impaga['fvcto'],
                              fuzzy=True,
                              dayfirst=True)

        except Exception as e:
            logging.error('{} error: {}'.format(
                DDATablesProcessor.get_fecha_mora.__qualname__, e))
    ...
    def get_valor_ultima_cuota(self) -> Union[int, None]:
        try:
            if self.fila_ultima_cuota_pagada is not None:
                return int(self.fila_ultima_cuota_pagada['valor_cuota'].replace('.', ''))
            return None
        except Exception as e:
            logging.error('{} error: {}'.format(
                DDATablesProcessor.get_valor_ultima_cuota.__qualname__, e))
...
class DDATextProcessor():
    ...
    def get_nombre(self) -> Union[str, None]:
        try:
            ...
            logging.error('{} error: {}'.format(DDATextProcessor.get_nombre.__qualname__,
                                                e))
            return None
    ...
    def get_rut(self) -> Union[str, None]:
        try:
            ...
            logging.error('{} error: {}'.format(DDATextProcessor.get_rut.__qualname__,
                                                e))
    ...
    def get_vendedor(self) -> Union[str, None]:
        try:
            rut_pattern = re.compile(r"(?<=contrato:)(.+?)(?=nombre)")
            found = rut_pattern.findall(self.text)

            return found[0].replace(':', '').strip().upper()
        except Exception as e:
            logging.error('{} error: {}'.format(
                DDATextProcessor.get_vendedor.__qualname__, e))
    ...
    def get_cuotas(self) -> Union[str, None]:
        try:
            ...
            logging.error('{} error: {}'.format(DDATextProcessor.get_cuotas.__qualname__,
                                                e))
    ...
    def get_tasa_interes(self) -> Union[float, None]:
        try:
            ...
            logging.error('{} error: {}'.format(
                DDATextProcessor.get_tasa_interes.__qualname__, e))
...
class DesarrolloContratosProcessor(Processor):
    def process_response(self, text: str, tables: Union[List[pd.DataFrame], None],
        ...
        self.processed_tables = DDATablesProcessor(tables or [])
        ...
        row = {
            ...
            self.processed_tables.get_fecha_pago_primera_cuota(),
            ...
            'Archivo Desarrollo Contratos': filename
        }

        errores = self.processed_tables.errors + self.processed_text.errors
        row['Errores Desarrollo Contratos'] = ', '.join(errores)
